...
const getContentsAndId = () => {
  ...
  const numOne = 1;
  const [lastElement] = contents.slice(-numOne);
  const id = lastElement ? lastElement.id + numOne : numOne;
  return { contents, id };
};

const addTaskId = (tasks, todoId) => {
  let taskId = 0;
  const idAddedTasks = tasks.map(task => {
    taskId++;
    task.id = `${todoId}-${taskId}`;
    return task;
  });
  return idAddedTasks;
};
...
const updateDatabase = (req, res) => {
  ...
  contents.push({
    id,
    title,
    tasks: addTaskId(JSON.parse(tasks), id),
    timeStamp
  });
...
const updateTaskStatus = (req, res) => {
  const idPosition = 5;
  const taskId = req.body.slice(idPosition);
  const contents = getPreviousContents(TODO_STORE);
  const updatedTodoList = contents.map(todo => {
    const { tasks } = todo;
  ...
  writeFileSync(TODO_STORE, JSON.stringify(updatedTodoList));
  res.end(JSON.stringify(updatedTodoList));
};
